...
ggtern(data = df1.coda, aes(x=Ca, Mg, Na)) + 
  geom_point(aes(col=Facie), alpha=0.25) + geom_density2d() + theme_classic()
```
...
```{r, fig.width=12, fig.height=5, echo=FALSE, comment=""}
grid.arrange(
  ggplot( data = df1.ilr ) +
  ...
  ggplot( data = df2.ilr ) +
    geom_point(aes(x=coord.1, y=coord.2, col=Facie), alpha=0.5) + 
    geom_density2d(aes(x=coord.1, y=coord.2)) + theme_classic(), ncol=2 )
```