...
ggmap(map) + 
  geom_point(data = data %>% mutate(g = mod@bestResult@partition), aes(x = lon, y = lat), alpha=0.4, col='black') +
  xlab(NULL) + ylab(NULL) +
  theme(axis.ticks = element_blank(), axis.text = element_blank()) + 
  facet_wrap(~g, ncol=3)
...
df$id = factor(df$id, 
               levels = unique(df$id)[order( (df %>% subset(balance == levels(df$balance)[1]))[['value']] )])
ggplot() + 
  geom_point(data=df %>% dplyr::arrange(balance,value), 
             aes(x=id, y=value, colour=id), size=3) +
